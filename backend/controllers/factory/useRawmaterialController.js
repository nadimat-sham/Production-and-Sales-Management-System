...
const Receive = require("../../models/factory/useRawmaterialModel");
...
const receiveProducts = async (req, res) => {
  ...
  try {
    ...
    await receive.save();
    res.status(200).json({ message: "Raw materials used successfully" });
  } catch (error) {
    res
      .status(500)
      .json({ error: "An error occurred while using raw materials" });
  }
};
...
const receiveHistory = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while fetching use history" });
  }
};
