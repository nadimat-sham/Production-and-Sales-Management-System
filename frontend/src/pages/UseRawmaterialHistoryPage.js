...
import ReceiveHistory from "../components/UseRawmaterialHistory";
...
const ReceiveHistoryPage = () => {
  ...
  useEffect(() => {
    const fetchReceiveHistory = async () => {
      try {
        const response = await axios.get("/factory/useRawmaterial/history/use");
    ...
    <div>
      <div className="fixed w-[1200px]">
        <div className=" py-4 bg-white pr-[100px] flex justify-end gap-3 mr-0 items-center ml-[0px] bg-opacity-100">
          <p className="text-2xl mr-10">Use Histories</p>
          <input
            type="text"
            placeholder="Search..."
            value={searchTerm}
          ...
          >
            ...
            <option value="product">Raw material</option>
