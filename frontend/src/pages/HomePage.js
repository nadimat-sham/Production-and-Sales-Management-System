...
import PieChartProfit from "../components/PieChartProfit";
...
const Home = () => {
  ...
  const [startingDateProfit, setStartingDateProfit] = useState(
    startDate.toISOString()
  );
  const [endingDateProfit, setEndingDateProfit] = useState(
    endDate.toISOString()
  );
  ...
  let expectedEndDays = labelss.map((label, index) => {
    ...
    if (expectedEndDay > 30) expectedEndDay = 30;
  ...
  const handleStartingDateProfitChange = (event) => {
    setStartingDateProfit(event.target.value);
  };

  const handleEndingDateProfitChange = (event) => {
    setEndingDateProfit(event.target.value);
  };
    ...
    <div className="ml-32">
      ...
      {expectedEndDays.length > 0 && (
        <div className="mt-20">
          ...
          <div className="bar-charts mt-10 w-[900px]  bg-white rounded-xl shadow-md overflow-hidden p-12">
            <h1 className="text-xl font-bold text-gray-800 text-center">
              Monthly Profit
            </h1>
            <div className="mt-10">
              <div className="flex justify-center">
                <div>
                  ...
                  <DatePicker
                    selected={startingDateProfit}
                    onChange={(date) => setStartingDateProfit(date)}
                  ...
                  <DatePicker
                    selected={endingDateProfit}
                    onChange={(date) => setEndingDateProfit(date)}
                    dateFormat="dd/MM/yyyy"
                    className="border-2 border-gray-300 bg-white h-10 px-5 pr-16 rounded-lg text-sm focus:outline-blue-400"
                  />
                </div>
              ...
              <div className="mt-8 flex justify-center">
                <div className="w-[600px]">
                  <PieChartProfit
                    products={products}
                    startingDate={startingDateProfit}
                    endingDate={endingDateProfit}
                  />
                </div>
              </div>
            </div>
