...
const EmployeeAccount = ({ employeeId }) => {
  ...
  const handleAddSalary = () => {
    axios.post(`/accounts/${employeeId}/add`)
      .then(response => {
  ...
  const handleWithdrawSalary = () => {
    axios.post(`/accounts/${employeeId}/withdraw`, { amount: withdrawalAmount })
      .then(response => {
        console.log(response.data);
        // Handle success
        fetchAccount();
  ...
  return (
    <div className=" border-gray-500 p-4 rounded-lg mt-4">
      ...
      <p className="mb-4 text-green-600 font-bold text-xl">Total Money: {account.totalMoney}</p>
    ...
    <div className='border-x border-t border-gray-500 rounded-lg m-4 p-4'>
      <div className="mt-2 flex justify-between">
                ...
                <div className='flex flex-col justify-center '>
                    <div className='font-bold text-center'>Status</div>
                    {
                        account.history.map(entry=>(
                            <div className={`${entry.type==="Credited" ? "text-green-700" : "text-red-700"} my-2 text-center`}>{entry.type}</div>
                        )
                        )
                    }
                </div>
                ...
                <div className='flex flex-col justify-center'>
                    ...
                    {
                        account.history.map(entry=>(
                            <div className=" my-2 text-center">{new Date(entry.date).toLocaleString('en-US',{
                                day: 'numeric',
                                month: 'short',
                                year: 'numeric',
                                hour: 'numeric',
